(function($, window) {
	...
	var itemTemplate = '<div class="mui-slider-item mui-zoom-wrapper {{className}}"><div class="mui-zoom-scroller"><img src="{{src}}" data-preview-lazyload="{{lazyload}}" style="{{style}}" class="mui-zoom"></div>{{content}}</div>';
	...
	var PreviewImage = function(options) {
		this.options = $.extend(true, {
			...
			footer: '<input class="commentsBtn" placeholder="写评论..."><p class="commentsFont"><i class="flaticon-edit"></i></p>'
		}, options || {});
		this.init();
		this.initEvent();
	};
	...
	proto.refresh = function(index, groupArray) {
		...
		for (var i = 0; from < to; from++, i++) {
			...
			if(itemData.el.getAttribute('data-content') && itemData.el.getAttribute('data-content')!='')
				itemStr = itemStr.replace('{{content}}', '<div class="mui-slider-img-content">'+itemData.el.getAttribute("data-content")+'</div>');
			else
